...
class SymbolicReference:
    ...
    def _check_ref_name_valid(ref_path: PathLike) -> None:
        ...
        for c in str(ref_path):
            if c in " ~^:?*[\\":
                ...
                if previous is None or previous == "/":
                    ...
                    raise ValueError(
                        f"Invalid reference '{ref_path}': references cannot contain '..'"
                    )
            elif c == "/":
                if previous == "/":
                    raise ValueError(
                ...
                raise ValueError(
                    f"Invalid reference '{ref_path}': references cannot contain ASCII control characters"
                )
        ...
        if previous == ".":
            raise ValueError(
                f"Invalid reference '{ref_path}': references cannot end with a period (.)"
            )
        elif previous == "/":
            raise ValueError(
                f"Invalid reference '{ref_path}': references cannot end with a forward slash (/)"
            )
        elif previous == "@" and one_before_previous is None:
            raise ValueError(
                f"Invalid reference '{ref_path}': references cannot be '@'"
            )
    ...
    def log_append(
        ...
        try:
            committer_or_reader: Union["Actor", "GitConfigParser"] = (
    ...
    def iter_items(
        ...
        **kwargs: Any,
        ...
        return (
            ...
            if r.__class__ is SymbolicReference or not r.is_detached
        )
