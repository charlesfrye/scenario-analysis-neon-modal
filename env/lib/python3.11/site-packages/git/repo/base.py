...
class Repo:
    ...
    def __init__(
        ...
        try:
            common_dir = (
                (Path(self.git_dir) / "commondir").read_text().splitlines()[0].strip()
            )
    ...
    def _get_config_path(
        self, config_level: Lit_config_levels, git_dir: Optional[PathLike] = None
    ) -> str:
        if git_dir is None:
    ...
    def config_writer(
        ...
        return GitConfigParser(
            self._get_config_path(config_level),
            read_only=False,
            repo=self,
    ...
    def blame(
        ...
        **kwargs: Any,
        ...
        data: bytes = self.git.blame(
            rev, *rev_opts, "--", file, p=True, stdout_as_string=False, **kwargs
        )
        ...
        **kwargs: Any,
        ...
        if not allow_unsafe_options:
            Git.check_unsafe_options(
                options=list(kwargs.keys()), unsafe_options=cls.unsafe_git_clone_options
            )
        if not allow_unsafe_options and multi_options:
            Git.check_unsafe_options(
                options=multi_options, unsafe_options=cls.unsafe_git_clone_options
            )
